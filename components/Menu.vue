<script setup lang="ts">
  ...
  const menuEntries = computed(() => {
    let entries = records
      .filter((record) => record.inMenu === true)
  ...
  const isInOpeningHours = (link) => {
    ...
    const gmtOffset = 1;
    const start = new Date(link.start).getHours() - gmtOffset;
    const end = new Date(link.end).getHours() - gmtOffset;

    return state.currentTime.getHours() >= start && state.currentTime.getHours() <= end;
  };
      ...
      <div class="max-container w-full">
        ...
        <div class="flex flex-col items-start gap-6">
          ...
          <div v-if="touchPoints?.length > 0" class="flex items-center gap-2">
            ...
            >
              <nuxt-icon :name="link.icon" class="text-2xl text-black" />
            </a>
          </div>
        </div>
      </div>
          ...
          <div class="hidden lg:block">
            <div class="flex items-center gap-4">
              <div v-if="touchPoints?.length > 0" class="flex items-center gap-1">
                ...
                >
                  <nuxt-icon :name="link.icon" class="text-2xl text-white group-hover:text-gold transition" />
                </a>
              </div>
              <div class="flex items-center gap-6 mx-3">
                <div
                  v-for="(menuEntry, index) in menuEntries"
                  v-show="menuEntry.inMenu"
                  class="relative"
                ...
                >
                  <NuxtLink
                    ...
                    :key="'menuEntry' + index"
                    class="flex select-none items-center transition-all px-1 py-1 rounded"
                  >
                    <div>
                      {{ menuEntry.title }}
                    </div>
